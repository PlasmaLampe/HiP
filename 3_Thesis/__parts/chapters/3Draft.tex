...
However, even these two simple use cases include complex interaction between the different controllers and views within the system. This is shown in the \ac{UML} sequence diagrams within Figure \ref{SD1} and Figure \ref{SD2}. Note that both \ac{UML} sequence diagrams does not contain the actual storing within the database to keep them on a manageable level of complexity/details. So, both diagrams contain only \emph{<<UI>>} and \emph{<<Controller>>} elements. Both \ac{UML} sequence diagrams show AngularJS partials as \emph{<<UI>>} elements. These partials are \ac{HTML} parts that can be exchanged and offer different \ac{UI} elements. The connection to the backend is done by the \emph{Play framework routing} controller, which redirects the \ac{HTTP} requests to the fitting controller.

\begin{figure}[th]
\centerline{\includegraphics[width=1\textwidth]{gfx/SDChangeContent}}
\caption{The corresponding \ac{UML} sequence diagram for use case \glqq\emph{Student changes content of a topic}\grqq.}
\label{SD1}
\end{figure} 
...
\caption{The corresponding \ac{UML} sequence diagram for use case \glqq\emph{Supervisor creates a new group}\grqq.}
\label{SD2}
\end{figure} 

\clearpage

Now, after we have seen what the application is about and got a first idea about used components within the \ac{UML} sequence diagrams, we will  take a closer look at the planned architecture.
...
However, because of the problematic file format, there may be a shift from the Junaio eco-system to alternative environments. The problems with the \emph{creator3dmap} are described in the following section.
