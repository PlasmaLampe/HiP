...
In general, the planned application should be able to handle the complete workflow for a university course situation. So, the backend should offer features that offer groups of students the work on specific topics. The domain model showing this situation for a general university course is shown in Figure \ref{domain}. 

\todo{model is wrong, describe the correct model more in the text}
...
A big problem within the software industry is that projects take longer than expected and the expectation of the customer differs more and more \todo{Quellen+ review} from the view of the development team because doing a 6 month part of requirements engineering, followed by an 18-month development period lets one finish a product that is already obsolete at the day it gets published.
